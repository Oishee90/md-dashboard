...
const Sidebar = () => {
    ...
    const isActiveO = location.pathname.startsWith("/odermanagement") || location.pathname.startsWith("/order-details");
    const isActiveA = location.pathname.startsWith("/addquestions") || location.pathname.startsWith("/question");
    const isActiveMakeAdmin = location.pathname.startsWith("/makeAdmin") ;
    const isActiveManage = location.pathname.startsWith("/managesubscription") || location.pathname.startsWith("/subscription") || location.pathname.startsWith("/coupon-code");
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between w-[280px] font-semibold">
                {/* Left Indicator Bar */}
                <div className={`p-[3px] h-[60px] rounded-r-2xl ${isActiveDashboard? "bg-[#8CAB91]" : ""}`}></div>

                {/* Main Button Area */}
                <div className={`flex items-center space-x-2 justify-start w-[250px] h-[60px] pl-[30px] 
    ...
    <NavLink 
        ...
        >
            <div className="flex items-center justify-between w-[280px] font-semibold">
                {/* Left Indicator Bar */}
                <div className={`p-[3px] h-[60px] rounded-r-2xl ${isActiveO? "bg-[#8CAB91]" : ""}`}></div>
        ...
        >
            <div className="flex items-center justify-between w-[280px] font-semibold">
                {/* Left Indicator Bar */}
                <div className={`p-[3px] h-[60px] rounded-r-2xl ${isActiveA? "bg-[#8CAB91]" : ""}`}></div>

                {/* Main Button Area */}
                <div className={`flex items-center space-x-2 justify-start w-[250px] h-[60px] pl-[30px] 
                    ${isActiveA? "bg-[#8CAB91] text-[#FAF1E6]" : ""}`}
                >
                    <RiShoppingBag3Line className="w-[18px] h-[18px]" />
        ...
        >
            <div className="flex items-center justify-between w-[280px] font-semibold">
                {/* Left Indicator Bar */}
                <div className={`p-[3px] h-[60px] rounded-r-2xl ${isActiveMakeAdmin? "bg-[#8CAB91]" : ""}`}></div>
    ...
    <NavLink 
        ...
        >
            <div className="flex items-center justify-between w-[280px] font-semibold">
                {/* Left Indicator Bar */}
                <div className={`p-[3px] h-[60px] rounded-r-2xl ${isActiveManage? "bg-[#8CAB91]" : ""}`}></div>
         ...
         className={`flex  relative items-center space-x-2 justify-start w-[250px] h-[60px] pl-[30px] 
                ...
                >
                    ...
                    <h1 className="text-[16px] font-[500] whitespace-nowrap">Manage Subscription</h1>
                    {isDropdownOpen ? (
              <FaChevronUp className={` dark:text-white w-[20px] h-[15px]   ${isActiveManage? "bg-[#8CAB91] text-[#FAF1E6]" : "text-black"} `}/>
            ) : (
              <FaChevronDown className={` dark:text-white w-[20px] h-[15px]   ${isActiveManage? "bg-[#8CAB91] text-[#FAF1E6]" : "text-black"} `} />
            )}
             {isDropdownOpen && (
        <ul  ref={dropdownRef} className="absolute left-[0%] top-[94%] mt-1 w-full bg-[#FAF1E6] border border-gray-300 shadow-lg z-10 text-center">
          <li>
            <NavLink
              to="/subscription"
              className="block  py-2 hover:bg-[#8CAB91] hover:text-[#FAF1E6] text-gray-700  "
            >
              Subscription
            </NavLink>
          </li>
        ...
        >
            <div className="flex items-center justify-between w-[280px] font-semibold">
                {/* Left Indicator Bar */}
                <div className={`p-[3px] h-[60px] rounded-r-2xl ${isActiveSettings? "bg-[#8CAB91]" : ""}`}></div>
